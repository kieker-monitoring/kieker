...
node('kieker-slave-docker') {
  try {    
    ...
    stage ('1-compile logs') {
        sh 'docker run --rm -u `id -u` -v ' + env.WORKSPACE + ':/opt/kieker kieker/kieker-build:openjdk8 /bin/bash -c "cd /opt/kieker; ./gradlew -S compileJava compileTestJava"'
    }

    stage ('2-unit-test logs') {
        sh 'docker run --rm -u `id -u` -v ' + env.WORKSPACE + ':/opt/kieker kieker/kieker-build:openjdk8 /bin/bash -c "cd /opt/kieker; ./gradlew -S test"'
        junit '**/build/test-results/test/*.xml'
    }

    stage ('3-static-analysis logs') {
        sh 'docker run --rm -u `id -u` -v ' + env.WORKSPACE + ':/opt/kieker kieker/kieker-build:openjdk8 /bin/bash -c "cd /opt/kieker; ./gradlew -S check"'    
    }

    stage ('4-release-check-short logs') {
        sh 'docker run --rm -u `id -u` -v ' + env.WORKSPACE + ':/opt/kieker kieker/kieker-build:openjdk8 /bin/bash -c "cd /opt/kieker; ./gradlew checkReleaseArchivesShort"'
    ...
    stage ('5-release-check-extended logs') {
        if (env.BRANCH_NAME == "master" || env.CHANGE_TARGET != null) {
            ...
            sh 'docker run --rm -u `id -u` -v ' + env.WORKSPACE + ':/opt/kieker kieker/kieker-build:openjdk8 /bin/bash -c "cd /opt/kieker; ./gradlew checkReleaseArchives"'
        } else {
    ...
    stage ('push-to-stable') {
	    ...
	    sh 'echo "Uploading snapshot archives to oss.sonatype.org."'
            withCredentials([usernamePassword(credentialsId: 'artifactupload', usernameVariable: 'kiekerMavenUser', passwordVariable: 'kiekerMavenPassword')]) {
                sh 'docker run --rm -u `id -u` -e kiekerMavenUser=$kiekerMavenUser -e kiekerMavenPassword=$kiekerMavenPassword -v ' + env.WORKSPACE + ':/opt/kieker kieker/kieker-build:openjdk8 /bin/bash -c "cd /opt/kieker; ./gradlew uploadArchives"'
            }
        } else {
