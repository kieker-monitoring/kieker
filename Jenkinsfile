  ...
  try {
    ...
    withEnv(['HOST_IP=' + hostIP]) {
      stage('integration-test logs') {
          ...
          sh 'docker run --rm -u root -v ' + env.WORKSPACE + ':/opt/kieker -v /var/run/docker.sock:/var/run/docker.sock:rw -v /usr/bin/docker:/usr/bin/docker ' + DOCKER_IMAGE_NAME + ' /bin/bash -c "cd /opt/kieker; ./gradlew -S integrationTest"'
      }
    }
