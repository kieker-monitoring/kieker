...
CSV_HEADER="configuration;iteration;threads;order_index;threadid;duration_nsec"
...
for ((i=1;i<=${NUM_LOOPS};i+=1)); do
  for ((j=1;j<=${THREADS};j+=1)); do
      ...
      java  ${JAVAARGS_INSTR_EMPTYPROBE} -cp "${CLASSPATH}" ${MAINCLASSNAME} \
          --output-filename ${RESULTSFN} \
          --configuration-id "empty_probe;${i};${j};1" \
          --totalcalls ${TOTALCALLS} \
          --recordedcalls ${RECORDEDCALLS} \
          --methodtime ${METHODTIME} \
          --totalthreads ${j}
      ...
      java  ${JAVAARGS_INSTR_NOLOGGING} -cp "${CLASSPATH}" ${MAINCLASSNAME} \
          --output-filename ${RESULTSFN} \
          --configuration-id "no_logging;${i};${j};2" \
          --totalcalls ${TOTALCALLS} \
          --recordedcalls ${RECORDEDCALLS} \
          --methodtime ${METHODTIME} \
          --totalthreads ${j}
      ...
      java  ${JAVAARGS_INSTR_LOGGING} -cp "${CLASSPATH}" ${MAINCLASSNAME} \
          --output-filename ${RESULTSFN} \
          --configuration-id "logging;${i};${j};3" \
          --totalcalls ${TOTALCALLS} \
          --recordedcalls ${RECORDEDCALLS} \
          --methodtime ${METHODTIME} \
          --totalthreads ${j}
