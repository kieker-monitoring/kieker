  ...
  for (cl in (1:configs.loop)) {
    ...
    for (cc in (1:configs.count)) {
      ...
      for (ci in (1:buckets.count)) {
        results.ts[cc,ci] <- buckets.size * 10000000 / sum(results[(((ci-1)*buckets.size)+1):(ci*buckets.size),"rt_musec"])
      }
    ...
    title(main=paste("Iteration: ", cl, "  Recursion Depth: ", cr),ylab="Throughput (calls/s)")
  }
}
invisible(dev.off())
