 ...
 * @since 1.3
 */
public class TestConfigurationFactoryMethods extends AbstractKickerTest {

	private static final String EXAMPLE_CONFIG_FILE_IN_TRUNK = "test/monitoring/META-INF/kicker.monitoring.test.properties";
	...
	public void testCreationDefaultConfiguration() {
		...
		Assert.assertEquals("kicker.monitoring.writer.filesystem.AsyncFsWriter", configuration.getStringProperty(ConfigurationFactory.WRITER_CLASSNAME));
		// TimeSource controller
		Assert.assertEquals("kicker.monitoring.timer.SystemNanoTimer", configuration.getStringProperty(ConfigurationFactory.TIMER_CLASSNAME));
		// Sampling controller
