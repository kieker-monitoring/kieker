...
public class ExplorVizReader extends AbstractReaderPlugin {
	...
	public boolean read() {
		...
		try {
			...
			long timestamp, traceID;
			int orderIndex;
			byte clazzid, notNeededClazzID;
			boolean flag = true;
			...
			while ((socketChannel.read(buffer) != -1) && (!this.terminated)) {
				...
				try {
					while (buffer.hasRemaining()) {
						...
						if (clazzid == 1) { // BeforeOperationEvent
							timestamp = buffer.getLong();
							traceID = buffer.getLong();
							orderIndex = buffer.getInt();
							final int notRelevant = buffer.getInt();
							final int opSinatureStringID = buffer.getInt();
							final int classSignatureStringID = buffer.getInt();
							final int emptyStringID3 = buffer.getInt();
						...
						} else if (clazzid == 2) { // AfterFailedOperationEvent
							timestamp = buffer.getLong();
							traceID = buffer.getLong();
							orderIndex = buffer.getInt();
							final int causeStringID = buffer.getInt();
							notNeededClazzID = buffer.get();
							RegistryRecord.registerRecordInRegistry(buffer, this.stringRegistry);
