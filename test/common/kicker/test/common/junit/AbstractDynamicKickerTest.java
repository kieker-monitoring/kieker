 ...
 * @since 1.9
 */
public abstract class AbstractDynamicKickerTest extends AbstractKickerTest {
	...
	protected Collection<Class<?>> deliverAllAvailableClassesFromSourceDirectory() throws ClassNotFoundException {
		return AbstractDynamicKickerTest.transformClassNameToClasses(AbstractDynamicKickerTest.transformFilesToClassNames(AbstractDynamicKickerTest.listSourceFiles(
	...
	protected Collection<Class<?>> deliverAllAvailableClassesFromTestDirectoryInJUnitPackage() throws ClassNotFoundException {
		return AbstractDynamicKickerTest.transformClassNameToClasses(AbstractDynamicKickerTest.transformFilesToClassNames(
				AbstractDynamicKickerTest.filterOutFilesNotMatchingFullQualifiedPathName(PATTERN_JUNIT_PACKAGE_NAME,
						AbstractDynamicKickerTest.listSourceFiles(DIR_NAME_TESTS, PATTERN_JAVA_TEST_FILES))));
	}
	...
	private static Collection<Class<?>> transformClassNameToClasses(final Collection<String> classNames) throws ClassNotFoundException {
		...
		for (final String className : classNames) {
			result.add(AbstractDynamicKickerTest.class.getClassLoader().loadClass(className));
		}
	...
	public Collection<Class<?>> filterOutClassesExtending(final Class<AbstractKickerTest> superClass, final Collection<Class<?>> classes) {
		final Collection<Class<?>> result = new LinkedList<Class<?>>();
