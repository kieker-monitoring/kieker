...
@Plugin(configuration = {
	@Property(name = GCTimeDisplayFilter.CONFIG_PROPERTY_NAME_NUMBER_OF_ENTRIES,
			defaultValue = GCTimeDisplayFilter.CONFIG_PROPERTY_VALUE_NUMBER_OF_ENTRIES) })
public class GCTimeDisplayFilter extends AbstractFilterPlugin {
	...
	private static final Log LOG = LogFactory.getLog(GCTimeDisplayFilter.class);
	...
	public GCTimeDisplayFilter(final Configuration configuration, final IProjectContext projectContext) {
		...
		this.numberOfEntries = configuration.getIntProperty(GCTimeDisplayFilter.CONFIG_PROPERTY_NAME_NUMBER_OF_ENTRIES);
		final String recordTimeunitProperty = projectContext.getProperty(IProjectContext.CONFIG_PROPERTY_NAME_RECORDS_TIME_UNIT);
		TimeUnit recordTimeunit;
		try {
			recordTimeunit = TimeUnit.valueOf(recordTimeunitProperty);
		} catch (final IllegalArgumentException ex) { // already caught in AnalysisController, should never happen
			GCTimeDisplayFilter.LOG.warn(recordTimeunitProperty + " is no valid TimeUnit! Using NANOSECONDS instead.");
	...
	@InputPort(name = GCTimeDisplayFilter.INPUT_PORT_NAME_RECORDS, eventTypes = { GCRecord.class })
	public synchronized void inputRecords(final GCRecord record) {
		this.records.add(record);
	}

	@InputPort(name = GCTimeDisplayFilter.INPUT_PORT_NAME_TIMESTAMPS, eventTypes = { Long.class })
	public synchronized void inputTimeEvents(final Long timestamp) {
		...
		for (final GCRecord record : this.records) {
			this.plot.setEntry(record.getGcName(), minutesAndSeconds, record.getCollectionTime());
		}
	...
	public Configuration getCurrentConfiguration() {
		...
		configuration.setProperty(GCTimeDisplayFilter.CONFIG_PROPERTY_NAME_NUMBER_OF_ENTRIES, String.valueOf(this.numberOfEntries));
