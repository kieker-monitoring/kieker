...
Created by Reiner Jung on Aug 08, 2019

Kieker comes with a wide range of samplers to collect resource
consumption. Currently, we use Sigar for these samplers. In future we
will use the oshi library, as Sigar is no longer developed.
...
means of injection, but we hadn't the time to get around to it and
implement it. Therefore, you have to create a proper injection setup and
use the SigarSamplerFactory to create samplers.
...
processes. The value should be in the range <code>[0,1]</code> \*/

double user = 0.0

/*\* Fraction of time during which the CPU was used by the kernel. The
value should be in the range <code>[0,1]</code> \*/
...
processes with a high nice value. The value should be in the range
<code>[0,1]</code> \*/
...
/*\* Fraction of time during which the CPU was utilized. Typically, this
is the sum of {@link #user}, {@link #system}, {@link #wait}, and {@link
#nice}. The value should be in the range <code>[0,1]</code> \*/

double totalUtilization = 0.0

/*\* Fraction of time during which the CPU was idle. The value should be
