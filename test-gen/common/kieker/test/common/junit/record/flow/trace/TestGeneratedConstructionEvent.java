...
public class TestGeneratedConstructionEvent extends AbstractGeneratedKiekerTest {
	...
	for (int i=0;i<ARRAY_LENGTH;i++) {
			// initialize
			ConstructionEvent record = new ConstructionEvent(LONG_VALUES[i%LONG_VALUES.length], LONG_VALUES[i%LONG_VALUES.length], INT_VALUES[i%INT_VALUES.length], STRING_VALUES[i%STRING_VALUES.length], INT_VALUES[i%INT_VALUES.length]);
			
			// check values
			Assert.assertEquals("ConstructionEvent.timestamp values are not equal.", LONG_VALUES[i%LONG_VALUES.length], record.getTimestamp());
			Assert.assertEquals("ConstructionEvent.traceId values are not equal.", LONG_VALUES[i%LONG_VALUES.length], record.getTraceId());
			Assert.assertEquals("ConstructionEvent.orderIndex values are not equal.", INT_VALUES[i%INT_VALUES.length], record.getOrderIndex());
			Assert.assertEquals("ConstructionEvent.classSignature values are not equal.", STRING_VALUES[i%STRING_VALUES.length] == null?"":STRING_VALUES[i%STRING_VALUES.length], record.getClassSignature());
			Assert.assertEquals("ConstructionEvent.objectId values are not equal.", INT_VALUES[i%INT_VALUES.length], record.getObjectId());
