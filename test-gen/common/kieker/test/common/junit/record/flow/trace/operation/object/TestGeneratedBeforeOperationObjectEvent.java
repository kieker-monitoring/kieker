...
public class TestGeneratedBeforeOperationObjectEvent extends AbstractGeneratedKiekerTest {
	...
	for (int i=0;i<ARRAY_LENGTH;i++) {
			// initialize
			BeforeOperationObjectEvent record = new BeforeOperationObjectEvent(LONG_VALUES[i%LONG_VALUES.length], LONG_VALUES[i%LONG_VALUES.length], INT_VALUES[i%INT_VALUES.length], STRING_VALUES[i%STRING_VALUES.length], STRING_VALUES[i%STRING_VALUES.length], INT_VALUES[i%INT_VALUES.length]);
			
			// check values
			Assert.assertEquals("BeforeOperationObjectEvent.timestamp values are not equal.", LONG_VALUES[i%LONG_VALUES.length], record.getTimestamp());
			Assert.assertEquals("BeforeOperationObjectEvent.traceId values are not equal.", LONG_VALUES[i%LONG_VALUES.length], record.getTraceId());
			Assert.assertEquals("BeforeOperationObjectEvent.orderIndex values are not equal.", INT_VALUES[i%INT_VALUES.length], record.getOrderIndex());
			Assert.assertEquals("BeforeOperationObjectEvent.classSignature values are not equal.", STRING_VALUES[i%STRING_VALUES.length] == null?"":STRING_VALUES[i%STRING_VALUES.length], record.getClassSignature());
			Assert.assertEquals("BeforeOperationObjectEvent.operationSignature values are not equal.", STRING_VALUES[i%STRING_VALUES.length] == null?"":STRING_VALUES[i%STRING_VALUES.length], record.getOperationSignature());
			Assert.assertEquals("BeforeOperationObjectEvent.objectId values are not equal.", INT_VALUES[i%INT_VALUES.length], record.getObjectId());
