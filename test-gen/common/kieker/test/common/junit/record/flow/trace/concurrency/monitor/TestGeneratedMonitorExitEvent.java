...
public class TestGeneratedMonitorExitEvent extends AbstractGeneratedKiekerTest {
	...
	for (int i=0;i<ARRAY_LENGTH;i++) {
			// initialize
			MonitorExitEvent record = new MonitorExitEvent(LONG_VALUES[i%LONG_VALUES.length], LONG_VALUES[i%LONG_VALUES.length], INT_VALUES[i%INT_VALUES.length], INT_VALUES[i%INT_VALUES.length]);
			
			// check values
			Assert.assertEquals("MonitorExitEvent.timestamp values are not equal.", LONG_VALUES[i%LONG_VALUES.length], record.getTimestamp());
			Assert.assertEquals("MonitorExitEvent.traceId values are not equal.", LONG_VALUES[i%LONG_VALUES.length], record.getTraceId());
			Assert.assertEquals("MonitorExitEvent.orderIndex values are not equal.", INT_VALUES[i%INT_VALUES.length], record.getOrderIndex());
			Assert.assertEquals("MonitorExitEvent.lockId values are not equal.", INT_VALUES[i%INT_VALUES.length], record.getLockId());
