...
public class TestGeneratedAfterOperationObjectEvent extends AbstractGeneratedKiekerTest {
	...
	for (int i=0;i<ARRAY_LENGTH;i++) {
			// initialize
			AfterOperationObjectEvent record = new AfterOperationObjectEvent(LONG_VALUES[i%LONG_VALUES.length], LONG_VALUES[i%LONG_VALUES.length], INT_VALUES[i%INT_VALUES.length], STRING_VALUES[i%STRING_VALUES.length], STRING_VALUES[i%STRING_VALUES.length], INT_VALUES[i%INT_VALUES.length]);
			
			// check values
			Assert.assertEquals("AfterOperationObjectEvent.timestamp values are not equal.", LONG_VALUES[i%LONG_VALUES.length], record.getTimestamp());
			Assert.assertEquals("AfterOperationObjectEvent.traceId values are not equal.", LONG_VALUES[i%LONG_VALUES.length], record.getTraceId());
			Assert.assertEquals("AfterOperationObjectEvent.orderIndex values are not equal.", INT_VALUES[i%INT_VALUES.length], record.getOrderIndex());
			Assert.assertEquals("AfterOperationObjectEvent.classSignature values are not equal.", STRING_VALUES[i%STRING_VALUES.length] == null?"":STRING_VALUES[i%STRING_VALUES.length], record.getClassSignature());
			Assert.assertEquals("AfterOperationObjectEvent.operationSignature values are not equal.", STRING_VALUES[i%STRING_VALUES.length] == null?"":STRING_VALUES[i%STRING_VALUES.length], record.getOperationSignature());
			Assert.assertEquals("AfterOperationObjectEvent.objectId values are not equal.", INT_VALUES[i%INT_VALUES.length], record.getObjectId());
