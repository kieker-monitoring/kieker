...
/**
 * Tests for kieker.common.record.flow.trace.concurrency.monitor.MonitorRequestEvent records.
 * 
 * @author Reiner Jung
 * 
 * @since 1.10
 */
public class TestMonitorRequestEventPropertyOrder extends AbstractKiekerTest {
	 ...
	 * Empty constructor.
	 */
	public TestMonitorRequestEventPropertyOrder() {}

	/**
	 * Test property order processing of CPUUtilizationRecord constructors.
	 */
	@Test
	public void testMonitorRequestEventPropertyOrder() { // NOPMD
		final IRegistry<String> stringRegistry = this.makeStringRegistry();
		final ByteBuffer inputBuffer = this.createByteBuffer(MonitorRequestEvent.SIZE, 
			this.makeStringRegistry(),
			PROPERTY_TIMESTAMP, PROPERTY_TRACE_ID, PROPERTY_ORDER_INDEX, PROPERTY_LOCK_ID);
		final Object[] values = { PROPERTY_TIMESTAMP, PROPERTY_TRACE_ID, PROPERTY_ORDER_INDEX, PROPERTY_LOCK_ID };
					
		final MonitorRequestEvent recordInitParameter = new MonitorRequestEvent(PROPERTY_TIMESTAMP, PROPERTY_TRACE_ID, PROPERTY_ORDER_INDEX, PROPERTY_LOCK_ID);
		final MonitorRequestEvent recordInitBuffer = new MonitorRequestEvent(inputBuffer, this.makeStringRegistry());
		final MonitorRequestEvent recordInitArray = new MonitorRequestEvent(values);
		
		this.assertMonitorRequestEvent(recordInitParameter);
		this.assertMonitorRequestEvent(recordInitBuffer);
		this.assertMonitorRequestEvent(recordInitArray);
		...
		final ByteBuffer outputBufferParameter = ByteBuffer.allocate(MonitorRequestEvent.SIZE);
		...
		final ByteBuffer outputBufferBuffer = ByteBuffer.allocate(MonitorRequestEvent.SIZE);
		...
		final ByteBuffer outputBufferArray = ByteBuffer.allocate(MonitorRequestEvent.SIZE);
	...
	/**
	 * Assertions for MonitorRequestEvent.
	 */
	private void assertMonitorRequestEvent(final MonitorRequestEvent record) {
