...
/**
 * Tests for kieker.common.record.flow.trace.concurrency.SplitEvent records.
 * 
 * @author Reiner Jung
 * 
 * @since 1.10
...
public class TestSplitEventPropertyOrder extends AbstractKiekerTest {
	 ...
	 * Empty constructor.
	 */
	public TestSplitEventPropertyOrder() {}

	/**
	 * Test property order processing of CPUUtilizationRecord constructors.
	 */
	@Test
	public void testSplitEventPropertyOrder() { // NOPMD
		final IRegistry<String> stringRegistry = this.makeStringRegistry();
		final ByteBuffer inputBuffer = this.createByteBuffer(SplitEvent.SIZE, 
			this.makeStringRegistry(),
			PROPERTY_TIMESTAMP, PROPERTY_TRACE_ID, PROPERTY_ORDER_INDEX);
		final Object[] values = { PROPERTY_TIMESTAMP, PROPERTY_TRACE_ID, PROPERTY_ORDER_INDEX };
					
		final SplitEvent recordInitParameter = new SplitEvent(PROPERTY_TIMESTAMP, PROPERTY_TRACE_ID, PROPERTY_ORDER_INDEX);
		final SplitEvent recordInitBuffer = new SplitEvent(inputBuffer, this.makeStringRegistry());
	...
	/**
	 * Assertions for SplitEvent.
	 */
	private void assertSplitEvent(final SplitEvent record) {
