...
import org.rosuda.JRclient.support.JCrypt;
...
public class Rconnection {
    ...
    */
    public Rconnection(final String host) throws RSrvException {
		this(host, 6311);
    }
	...
	@param port TCP port
    */
    public Rconnection(final String host, final int port) throws RSrvException {
		this(host, port, null);
    }

    Rconnection(final RSession session) throws RSrvException {
		this(null, 0, session);
    }
			...
			try {
				...
				for (int i = 12;i < 32;i += 4) {
					...
					if (attr.compareTo("ARpt") == 0 ) {
						if (!this.authReq) { // this method is only fallback when no other was specified
							this.authReq = true;
							this.authType = AT_plain;
						}
					}
