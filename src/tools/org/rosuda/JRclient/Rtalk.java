...
public class Rtalk {
	...
	public Rtalk(final InputStream sis, final OutputStream sos) {
		this.is = sis;
		this.os = sos;
	}
	 ...
	 */
	public static void setInt(final int v, final byte[] buf, int o) {
		buf[o] = (byte) (v & 255);
		o++;
	 ...
	 */
	public static int setHdr(final int ty, final int len, final byte[] buf, int o) {
	 ...
	 *            length
	 */
	public static byte[] newHdr(final int ty, final int len) {
		final byte[] hdr = new byte[(len > 0xfffff0) ? 8 : 4];
	 ...
	 */
	public static int getInt(final byte[] buf, final int o) {
	 ...
	 * @return length
	 */
	public static int getLen(final byte[] buf, final int o) {
	 ...
	 * @return long value
	 */
	public static long getLong(final byte[] buf, final int o) {
		final long low = (Rtalk.getInt(buf, o)) & 0xffffffffL;
		long hi = (Rtalk.getInt(buf, o + 4)) & 0xffffffffL;
		hi <<= 32;
		hi |= low;
		return hi;
	}

	public static void setLong(final long l, final byte[] buf, final int o) {
	 ...
	 */
	public Rpacket request(final int cmd) {
	 ...
	 * @return returned packet or <code>null</code> if something went wrong
	 */
	public Rpacket request(final int cmd, final byte[] cont) {
		return this.request(cmd, null, cont, 0, (cont == null) ? 0 : cont.length);
	}

	/**
	 * sends a request with attached prefix and parameters. Both prefix and cont can be <code>null</code>. Effectively <code>request(a,b,null)</code> and
	 * <code>request(a,null,b)</code> are equivalent.
	 * 
	 * @param cmd
	 *            command - a special command of -1 prevents request from sending anything
	 * @param prefix
	 *            - this content is sent *before* cont. It is provided to save memory copy operations where a small header precedes a large data chunk (usually
	 *            prefix conatins the parameter header and cont contains the actual data).
	 * @param cont
	 *            contents
	 * @param offset
	 ...
	 * @return returned packet or <code>null</code> if something went wrong
	 */
	public Rpacket request(final int cmd, final byte[] prefix, byte[] cont, int offset, int len) {
		if (cont != null) {
		...
		try {
			...
			final byte[] ih = new byte[16];
			if (this.is.read(ih) != 16) {
				return null;
			}
			final int rep = Rtalk.getInt(ih, 0);
			final int rl = Rtalk.getInt(ih, 4);
			if (rl > 0) {
				final byte[] ct = new byte[rl];
				int n = 0;
				while (n < rl) {
					final int rd = this.is.read(ct, n, rl - n);
					n += rd;
				}
	 ...
	 * @return returned packet or <code>null</code> if something went wrong
	 */
	public Rpacket request(final int cmd, final String par) {
		try {
			final byte[] b = par.getBytes(Rconnection.transferCharset);
			int sl = par.length() + 1;
			if ((sl & 3) > 0) {
				sl = (sl & 0xfffffc) + 4; // make sure the length is divisible by 4
			}
			final byte[] rq = new byte[sl + 5];
			int i;
			...
			;
			Rtalk.setHdr(DT_STRING, sl, rq, 0);
	 ...
	 * @return returned packet or <code>null</code> if something went wrong
	 */
	public Rpacket request(final int cmd, final int par) {
		try {
			final byte[] rq = new byte[8];
			Rtalk.setInt(par, rq, 4);
			Rtalk.setHdr(DT_INT, 4, rq, 0);
			return this.request(cmd, rq);
		} catch (final Exception e) {
		}
		;
