...
public class RFileOutputStream extends OutputStream {
	 ...
	 * Any subsequent read requests on closed stream result in an {@link IOException} or error result
	 */
	boolean closed;
	 ...
	 * 
	 * @param rti
	 *            RTalk object for communication with Rserve
	 * @param fb
	 *            filename of the file to create (existing file will be overwritten)
	 */
	RFileOutputStream(final Rtalk rti, final String fn) throws IOException {
		this.rt = rti;
		final Rpacket rp = this.rt.request(Rtalk.CMD_createFile, fn);
		if ((rp == null) || !rp.isOk()) {
	...
	/**
	 * writes one byte to the file. This function should be avoided, since {@link RFileOutputStream} provides no buffering. This means that each
	...
	public void write(final byte[] b, final int off, int len) throws IOException {
		...
		final boolean isLarge = (len > 0xfffff0);
		final byte[] hdr = Rtalk.newHdr(Rtalk.DT_BYTESTREAM, len);
		final Rpacket rp = this.rt.request(Rtalk.CMD_writeFile, hdr, b, off, len);
		if ((rp == null) || !rp.isOk()) {
			throw new IOException((rp == null) ? "Connection to Rserve failed" : ("Request return code: " + rp.getStat()));
		}
	}
	...
	public void close() throws IOException {
		final Rpacket rp = this.rt.request(Rtalk.CMD_closeFile, (byte[]) null);
		if ((rp == null) || !rp.isOk()) {
			throw new IOException((rp == null) ? "Connection to Rserve failed" : ("Request return code: " + rp.getStat()));
		}
		this.closed = true;
	...
	/**
	 * currently (Rserve 0.3) there is no way to force flush on the remote side, hence this function is noop. Future versions of Rserve may support this feature
	 * though. At any rate, it is safe to call it.
	 */
	@Override
	public void flush() {}
}
