/***************************************************************************
 * Copyright 2014 Kicker Project (http://kickerMonitoringController-monitoring.net)
 *
...
/**
 * Container for the Kicker Data Bridge handling the startup and shutdown of Kicker and the service connector.
 * 
 * @author Reiner Jung
 * @since 1.8
 */
public class ServiceContainer {
	...
	private final IMonitoringController kickerMonitoringController;
	...
	/**
	 * @param configuration
	 *            A configuration object for Kicker monitoring
	 * @param service
	...
	public ServiceContainer(final Configuration configuration, final IServiceConnector service, final boolean respawn) {
		this.kickerMonitoringController = MonitoringController.createInstance(configuration);
		this.respawn = respawn;
		this.service = service;
	}

	/**
	 * Main loop of the Kicker bridge.
	 * 
	...
	public void run() throws ConnectorDataTransmissionException {
		do {
			...
			while (this.active) {
				try {
					this.kickerMonitoringController.newMonitoringRecord(this.service.deserializeNextRecord());
					if ((this.kickerMonitoringController.getNumberOfInserts() % this.listenerUpdateInterval) == 0) {
		...
		this.kickerMonitoringController.terminateMonitoring();
	}
	...
	private void updateState(final String message) {
		for (final IServiceListener listener : this.listeners) {
			listener.handleEvent(this.kickerMonitoringController.getNumberOfInserts(), message);
		}
	}
	...
	public long getRecordCount() {
		return this.kickerMonitoringController.getNumberOfInserts();
	}
