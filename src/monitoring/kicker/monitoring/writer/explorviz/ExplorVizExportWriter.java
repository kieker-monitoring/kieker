...
final class ExplorVizExportWriterThread extends AbstractAsyncThread {
	...
	protected void consume(final IMonitoringRecord monitoringRecord) throws Exception {
		// sizes from ExplorViz not Kicker!
		int recordSize = 0;
		...
		this.convertKickerToExplorViz(buffer, monitoringRecord);
	...
	private void convertKickerToExplorViz(final ByteBuffer buffer, final IMonitoringRecord kickerRecord) {
		if (kickerRecord instanceof BeforeOperationEvent) {
			final BeforeOperationEvent kickerBefore = (BeforeOperationEvent) kickerRecord;
			buffer.put(BEFORE_OPERATION_CLAZZ_ID);
			buffer.putLong(kickerBefore.getTimestamp());
			buffer.putLong(kickerBefore.getTraceId());
			buffer.putInt(kickerBefore.getOrderIndex());
			if (kickerRecord instanceof IObjectRecord) {
				final IObjectRecord iObjectRecord = (IObjectRecord) kickerRecord;
				buffer.putInt(iObjectRecord.getObjectId());
			} else {
				buffer.putInt(0);
			}
			buffer.putInt(this.monitoringController.getUniqueIdForString(kickerBefore.getOperationSignature()));
		} else if (kickerRecord instanceof AfterOperationFailedEvent) {
			final AfterOperationFailedEvent kickerAfterFailed = (AfterOperationFailedEvent) kickerRecord;
			buffer.put(AFTER_FAILED_OPERATION_CLAZZ_ID);
			buffer.putLong(kickerAfterFailed.getTimestamp());
			buffer.putLong(kickerAfterFailed.getTraceId());
			buffer.putInt(kickerAfterFailed.getOrderIndex());
			buffer.putInt(this.monitoringController.getUniqueIdForString(kickerAfterFailed.getCause()));
		} else if (kickerRecord instanceof AfterOperationEvent) {
			final AfterOperationEvent kickerAfter = (AfterOperationEvent) kickerRecord;
			buffer.put(AFTER_OPERATION_CLAZZ_ID);
			buffer.putLong(kickerAfter.getTimestamp());
			buffer.putLong(kickerAfter.getTraceId());
			buffer.putInt(kickerAfter.getOrderIndex());
		} else if (kickerRecord instanceof RegistryRecord) {
			final RegistryRecord registryRecord = (RegistryRecord) kickerRecord;
