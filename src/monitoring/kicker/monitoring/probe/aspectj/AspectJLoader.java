...
public final class AspectJLoader {
	...
	public static void premain(final String options, final Instrumentation instrumentation) {
		if (!AspectJLoader.checkConfigurationFileAvailable()) {
			...
			LOG.info("No AspectJ configuration file found. Using Kicker's default AspectJ configuration file (" + aspectConfigURL + ").");
			AspectJLoader.addKickerDefaultConfigFile();
		}
	...
	private static boolean checkConfigurationFileAvailable() {
		...
		LOG.info("Using Kicker's AspectJLoader. This is not recommended for multi-classloader environments such as JavaEE and OSGI. Use the additional VM"
				+ " parameter '-D" + KIEKER_MONITORING_SKIP_DEFAULT_AOP_CONFIGURATION + "=true'. to disable Kicker's AspectJLoader");
	...
	private static void addKickerDefaultConfigFile() {
