...
public abstract class AbstractOperationExecutionMethodInvocationInterceptor
		implements MethodInterceptor, IMonitoringProbe {

	private static final Log log = LogFactory
			.getLog(AbstractOperationExecutionMethodInvocationInterceptor.class);

	protected static final MonitoringController tpmonController = MonitoringController
			.getInstance();
	protected static final SessionRegistry sessionRegistry = SessionRegistry
			.getInstance();
	...
	protected OperationExecutionRecord initExecutionData(
		...
		if (this.useRuntimeClassname) {
			...
			componentName = invocation.getMethod().getDeclaringClass()
					.getName();
		}

		final OperationExecutionRecord execData = new OperationExecutionRecord(
				componentName /* component */, opname /* operation */, traceId /*
																			 * -1
																			 * if
																			 * entry
																			 * point
		...
		execData.experimentId = AbstractOperationExecutionMethodInvocationInterceptor.tpmonController
				.getExperimentId();
	...
	protected void proceedAndMeasure(final MethodInvocation invocation,
			final OperationExecutionRecord execData) throws Throwable {
		execData.tin = AbstractOperationExecutionMethodInvocationInterceptor.tpmonController
				.currentTimeNanos();
		try {
			...
			execData.tout = AbstractOperationExecutionMethodInvocationInterceptor.tpmonController
					.currentTimeNanos();
