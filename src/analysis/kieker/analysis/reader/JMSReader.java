...
public class JMSReader extends AbstractMonitoringLogReader {
	...
	public JMSReader(final String jmsProviderUrl, final String jmsDestination) {
		this.initInstanceFromArgs(jmsProviderUrl, jmsDestination); // throws
	...
	public boolean init(final String initString) {
		try {
			...
			final String jmsProviderUrlP = propertyMap.getProperty(
					"jmsProviderUrl", null);
			final String jmsDestinationP = propertyMap.getProperty(
					"jmsDestination", null);
			this.initInstanceFromArgs(jmsProviderUrlP, jmsDestinationP); // throws
																			// IllegalArgumentException
	...
	private void initInstanceFromArgs(final String jmsProviderUrl,
		...
		if ((jmsProviderUrl == null) || jmsProviderUrl.equals("")
			...
			throw new IllegalArgumentException(
					"JMSReader has not sufficient parameters. jmsProviderUrl or jmsDestination is null");
		}
	...
	public boolean read() {
		...
		try {
			...
			final Destination destination = (Destination) context
					.lookup(this.jmsDestination);
			JMSReader.log.info("\n\n***\nListening to destination:"
					+ destination + " at " + this.jmsProviderUrl
					+ " !\n***\n\n");
			final MessageConsumer receiver = session
			...
			System.out
					.println("JMSTestListener1 is started and waits for incomming monitoring events!");
			...
			System.out.println("" + JMSReader.class.getName() + " "
					+ ex.getMessage());
			ex.printStackTrace();
			retVal = false;
		}
