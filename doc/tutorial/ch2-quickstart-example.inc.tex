...
\includegraphics[scale=0.9]{images/kieker_bookstoreclassdiagram}
\includegraphics[scale=0.9]{images/kieker_bookstoresequencediagram}
\end{figure}
...
.1 \DirInDirTree{example/}. %\DTcomment{The root directory of the project}.
.2 \DirInDirTree{build/}\DTcomment{Directory for the Java class files}. 
.2 \newFilesDirTreeFormat \DirInDirTree{lib/}\DTcomment{Directory for the required libraries}.
...
\WARNBOX{Under Windows it is also necessary to seperate the different paths for the classpath with a semicolon instead of a colon.
