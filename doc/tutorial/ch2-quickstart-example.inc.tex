...
\TODO{Bessere Methode, als die Quellcodedateien so "plain" zu nutzen.}
...
.1 example/. %\DTcomment{The root directory of the project}.
.2 build/\DTcomment{Directory for the Java class files}. 
.2 \color{blue}lib/\DTcomment{Directory for the required libraries}.
.3 \color{blue}\monitoringJar.
.3 \color{blue}\commonJar.
.3 \color{blue}\commonsLoggingJar.
.2 src/\DTcomment{The directory for the source code files}.
.3 \ldots.
}
% Linebreak because the text would be to close to the directory tree. 		
...
\TODO{Erneuter Hinweis, dass unter Windows die Quelldateien ausgeschrieben werden muessen.}
