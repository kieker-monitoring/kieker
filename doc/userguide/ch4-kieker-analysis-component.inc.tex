%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Kicker Analysis Component
%
% $Date$
% $Rev$:
% $Author$

\chapter{\KickerAnalysisPart{} Component}\label{chap:componentsAnalysis}
...
\KickerAnalysisPart{} provides a framework to define and execute pipe-and-filter %
...
\includegraphics[width=\textwidth]{images/kicker_AnalysisControlleruserguide-modified}
\caption{Class diagram showing important \KickerAnalysisPart{} types and their relationship}
...
with the important \KickerAnalysisPart{} classes and their relationships. %
Note that only the most important methods are included. 
An analysis with \KickerAnalysisPart{} is set up and executed employing %
the class \class{AnalysisController}. %
Setting up and running an analysis with \KickerAnalysisPart{} requires the %
...
\class{AnalysisController}. \KickerAnalysisPart{} configurations can be saved %
...
{\customComponentsBookstoreApplicationDir/src/kicker/examples/userguide/ch3and4bookstore/Starter.java}
...
\begin{figure}\centering
\includegraphics[scale=0.7]{images/kicker_readerimplsuserguide-modified}
\caption{Monitoring reader plugins included with \Kicker{}}
...
\noindent There are already some readers implemented in \Kicker,  as shown in the %
...
\Kicker{} already includes some basic filter plugins. For example, the %
...
the \KickerTraceAnalysis{} tool, presented in Chapter~\ref{chap:aspectJ}. %
...
% \pagebreak
% \setJavaCodeListing
%\lstinputlisting[caption=MyReponseTimeConsumer.java,label=lst:MyReponseTimeConsumer,firstline=21,firstnumber=21]{\customComponentsBookstoreApplicationDir/src/kicker/examples/userguide/ch3and4bookstore/MyResponseTimeConsumer.java}

% The following Listing~\ref{listing:AnalysisController} shows how to create and run an analysis %
% with these custom components:
%
% \setJavaCodeListing
% \lstinputlisting[caption=Code snippet setting up and running a \KickerAnalysisPart{} instance (Starter.java),label=listing:AnalysisController,firstline=48, lastline=82, firstnumber=48]%
% {\customComponentsBookstoreApplicationDir/src/kicker/examples/userguide/ch3and4bookstore/Starter.java}
...
Currently, \Kicker{} includes a single repository, \class{SystemModelRepository}, %
which is used by the \KickerTraceAnalysis{} filters to update and query %
...
When using components of the \KickerTraceAnalysis{}, make sure that write access to the %
...
% \Kicker{} provides input and output port interfaces and implementing classes %
% to implement such analyses. See the documentation of the classes \class{AbstractInputPort} %
% and \class{OutputPort} for details. \KickerTraceAnalysis{} is implemented %
% based on this pattern.

\subsection{Configuration}\label{sec:analysis:configuration}

\noindent According to the %
configuration of the \KickerMonitoringPart{} components (see Section~\ref{sec:monitoring:configuration}),
...
{\customComponentsBookstoreApplicationDir/src/kicker/examples/userguide/ch3and4bookstore/MyResponseTimeFilter.java}
...
% \setJavaCodeListing
% \lstinputlisting[firstline=29, firstnumber=29, caption=MyPipeReader.java (excerpt), label=listing:MyReader,float]{\customComponentsBookstoreApplicationDir/src/kicker/examples/userguide/ch3and4bookstore/MyPipeReader.java}
...
{\customComponentsBookstoreApplicationDir/src/kicker/examples/userguide/ch3and4bookstore/MyPipeReader.java}
...
of the \class{SystemModelRepository}, which is included in \Kicker{} as part of the %
\KickerTraceAnalysis{} tool. %

\enlargethispage{1cm}

\setJavaCodeListing
\lstinputlisting[firstline=43, firstnumber=43, lastline=46, caption={\class{@Respository} annotation of Kicker's \class{SystemModelRepository}}, label=listing:RepositoryAnnotation]%
{../../src/tools/kicker/tools/traceAnalysis/systemModel/repository/SystemModelRepository.java}
...
\KickerTraceAnalysis{} filter in Listing~\ref{listing:RepositoryRequirementDeclaration}. %
...
{../../src/tools/kicker/tools/traceAnalysis/filter/AbstractTraceAnalysisFilter.java}
