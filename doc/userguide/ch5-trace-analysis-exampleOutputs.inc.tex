...
The deployment component dependency, assembly component dependency, deployment operation dependency and the assembly operation dependency graphs can also be decorated with the response times, adding the minimum, the maximum and the average response times of the components. The decoration will be generated with the additional command line parameter \OPT{responseTimes} behind the corresponding \OPT{plot-}command.  An exemplaric graph with response times can be seen in Figure~\ref{fig:appendix:traceAnalysisExample:graphWithRespTimes}.
