...
\dir{WEB-INF/classes/META-INF/} directory, where it is considered for %
...
\setXMLListing
\lstinputlisting[firstline=50,lastline=61, numbers=none, linewidth=1\textwidth, caption=Enabling the Servlet filter in \file{applicationContext.xml}]{\JavaEEServletExampleDir/jetty-hightide-jpetstore/webapps/jpetstore/WEB-INF/web.xml}
...
\setXMLListing
\lstinputlisting[firstline=38,lastline=43, numbers=none, linewidth=1\textwidth, caption=Enabling the Spring interceptor in \file{applicationContext.xml}]{\JavaEEServletExampleDir/jetty-hightide-jpetstore/webapps/jpetstore/WEB-INF/applicationContext.KiekerSpringProbesActivated.xml}
...
\lstinputlisting[firstline=33,lastline=33, linewidth=1\textwidth,caption=Enabling the AspectJ weaver in Jetty's \file{start.ini}]{\JavaEEServletExampleDir/jetty-hightide-jpetstore/start.KiekerAspectJActivated.ini}

The AspectJ configuration file \file{aop.xml} is already located in the %
webapps's \dir{WEB-INF/classes/META-INF/} directory and configured to instrument %
...
\item Now, you can access the JPetStore application by opening the URL
   ...
   \Kieker{} intialization messages should appear in the console output. %
   The output includes the information where the monitoring data is written to
   (should be a \dir{kieker-<DATE-TIME>/} directory) located in the default %
   temporary directory.
