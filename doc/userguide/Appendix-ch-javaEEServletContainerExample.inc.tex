...
The subdirectory \file{jetty-hightide-jpetstore/} of the example includes the %
Jetty server with the JPetStore application already deployed to the server's %
\file{webapps} directory. In order to instrument the JPetStore with %
\KiekerMonitoringPart{}, the following files are relevant:

\begin{compactenum}
 \item \file{\aspectJWeaverJar{}}, \file{\mainJar}: These files are already %
included in the webapps's \dir{WEB-INF/lib/} directory.
 \item \file{\monitoringPropertiesFile}: The Kieker configuration file is %
already included in the webapps's \dir{WEB-INF/classes/META-INF} directory, %
where it is considered for configuration by \KiekerMonitoringPart{}. 
 \item \file{start.ini}: This file, located in Jetty's root directory, allows %
...
\lstinputlisting[firstline=33,lastline=35, linewidth=0.94\textwidth,caption=Enabling the AspectJ weaver in Jetty's \file{start.ini}]{\JavaEEServletExampleDir/jetty-hightide-jpetstore/start.KiekerAspectJActivated.ini}

 \item \file{aop.xml}: The AspectJ configuration is located in the %
webapps's \dir{WEB-INF/classes/META-INF} directory, configured to instrument %
the JPetStore classes with Kieker's \class{OperationExecutionAspectFull} aspect %
(Section~\ref{chap:aspectJ}).
\item \file{applicationContext.xml}: In this file, located in the webapps's %
\dir{WEB-INF} directory, Kieker's Spring interceptor can be enabled: 

\setXMLListing
\lstinputlisting[firstline=38,lastline=43, numbers=none, linewidth=0.94\textwidth, caption=Enabling the Spring interceptor in \file{applicationContext.xml}]{\JavaEEServletExampleDir/jetty-hightide-jpetstore/webapps/jpetstore/WEB-INF/applicationContext.KiekerSpringProbesActivated.xml}

 \item \file{web.xml}: In this file, located in the webapps's %
\dir{WEB-INF} directory, Kieker's Servlet filters are enabled: 

\setXMLListing
\lstinputlisting[firstline=50,lastline=61, numbers=none, linewidth=0.94\textwidth, caption=Enabling the Servlet filter in \file{applicationContext.xml}]{\JavaEEServletExampleDir/jetty-hightide-jpetstore/webapps/jpetstore/WEB-INF/web.xml}
\end{compactenum}
