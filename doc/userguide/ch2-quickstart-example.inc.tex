...
bookstore with a marketplace facility where users can search for books in an %
...
\WARNBOX{%
Unter Windows it is necessary to separate the classpath elements by %
...
file named \dir{tpmon.map} which contains information on the types of the monitoring records used. 
...
For the data analysis the \method{newMonitoringRecord()} (see lines 24--41) is used. %
...
The analysis started by calling its \method{run} method (line~24). %
