...
This chapter provides a brief introduction to \Kicker{} based on a simple Bookstore example application. Section~\ref{sec:example:downloadInstall} explains how to download and install \Kicker{}. The Bookstore application itself is introduced in Section~\ref{sec:example:bookstore}, while the following sections demonstrate how to use \Kicker{} for monitoring~(Section~\ref{sec:example:monitoring}) and analyzing~(Section~\ref{sec:example:analysis}) the resulting monitoring data.

%%
\section{Download and Installation}\label{sec:example:downloadInstall}

The \Kicker{} download site\footnote{\KickerDownloadURL{}} provides archives of the binary and source distribution, the Javadoc~API, as well as additional examples. For this quick start guide, \Kicker{}'s binary distribution, e.g., \file{\binaryFileForDownload}, is required and must be downloaded. After having extracted the archive, you'll find the directory structure and contents shown in Figure~\ref{fig:binary-layout}.
...
\begin{graybox}
\dirtree{%
  .1 \DirInDirTree{\KickerDir/}.
    .2 \DirInDirTree{bin/}\DTcomment{Call scripts for \Kicker{} tools}.
      .3 \ldots.
    .2 \DirInDirTree{dist/}\DTcomment{The \Kicker{} framework libraries}.
      .3 \mainJar.
      .3 \ldots. %\servletWar.
    .2 \DirInDirTree{doc/}\DTcomment{}.
      .3 kicker-\version\_userguide.pdf\DTcomment{PDF file of this document}.
    .2 \DirInDirTree{examples/}.
      .3 \DirInDirTree{userguide/}\DTcomment{Source code of the examples in this document}.
                        .4 \ldots.
    .2 \DirInDirTree{lib/}\DTcomment{Libraries required by \Kicker{}}.
%       .3 \commonsLoggingJar.
      .3 \ldots.
    .2 \DirInDirTree{META-INF/}\DTcomment{Example configuration files}.
      .3 \kickerMonitoringProperties{}.
      .3 \ldots.
}
\end{graybox}
\caption{Directory structure and contents of \Kicker{}'s binary distribution}
\label{fig:binary-layout}
\end{figure}

\pagebreak

The Java sources presented in this user guide, as well as pre-compiled binaries, are included in the \file{\exampleDir/} directory. The file \file{\mainJar{}} contains the \KickerMonitoringPart{} and \KickerAnalysisPart{} components, as well as the \KickerTraceAnalysis{} tool. The sample \KickerMonitoringPart{}
configuration file %
\file{\kickerMonitoringProperties{}} will be detailed in Chapter~\ref{chap:componentsMonitoring}. %
...
% Since \Kicker{} uses the Apache Commons library~\cite{CommonsLogging-WebSite} as a logging interface, the file \file{\commonsLoggingJar} is the only dependency to a third-party library which is needed to execute \Kicker{} in any case.
...
\includegraphics[scale=0.83]{images/kicker_bookstoreclassdiagram}%
}%
\subfigure[]{\label{fig:boostore:sequencediagram}%
\includegraphics[scale=0.83]{images/kicker_SequenceDiagram-manually-changed}%
}
...
\setJavaCodeListing
\lstinputlisting[caption=\method{main} method from BookstoreStarter.java,firstline=23,firstnumber=23,lastline=29,label=lst:class:BookstoreStarter]{\plainBookstoreApplicationDir/src/kicker/examples/userguide/ch2bookstore/BookstoreStarter.java}
...
\lstinputlisting[caption=Bookstore.java,label=lst:class:Bookstore,firstline=19,firstnumber=19]{\plainBookstoreApplicationDir/src/kicker/examples/userguide/ch2bookstore/Bookstore.java}
...
\lstinputlisting[caption=CRM.java,label=lst:class:CRM,firstline=19,firstnumber=19]{\plainBookstoreApplicationDir/src/kicker/examples/userguide/ch2bookstore/CRM.java}
...
\lstinputlisting[caption=Catalog.java,label=lst:class:Catalog,firstline=19,firstnumber=19]{\plainBookstoreApplicationDir/src/kicker/examples/userguide/ch2bookstore/Catalog.java}
...
> mkdir build
> javac src/kicker/examples/userguide/ch2bookstore/*.java -d build
...
\noindent In this section, the \Kicker{} example application was introduced and when everything went well, the bookstore is a runnable program. Furthermore, the composition of the application and its function should now be present. %
The next Section~\ref{sec:example:monitoring} will demonstrate how %
to monitor this example application employing \KickerMonitoringPart{} using manual instrumentation.

\pagebreak

%%
\section{Monitoring with \KickerMonitoringPart{}}\label{sec:example:monitoring}

In the previous Sections~\ref{sec:example:downloadInstall} and \ref{sec:example:bookstore}, the \Kicker{} installation and the example application have been introduced. In this section, the preparations for application monitoring, the instrumentation of the application, and the actual monitoring are explained.
...
hardly maintainable. \Kicker{} includes probes %
...
\noindent The first step is to copy the \Kicker{} jar-file \file{\mainJar} to the \dir{lib/} directory of the example directory~(see Section~\ref{sec:example:bookstore}). The file is located in the \dir{\KickerDir/dist/} directory of the extracted \Kicker{} archive, as described in Section~\ref{sec:example:downloadInstall}. %
The final layout of the example directory is illustrated in Figure~\ref{fig:KickerBookstoreExample}.
...
\end{graybox}
\caption{The directory structure of the Bookstore application with \Kicker{} libraries}
\label{fig:KickerBookstoreExample}
\end{figure}
...
\noindent \Kicker{} maintains monitoring data as  so-called monitoring records. %
...
is included in the \Kicker{} distribution. %
...
\begin{figure}[H]
\begin{centering}
\includegraphics[scale=1]{images/kicker_OperationExecutionRecord-notraceattributes-inheritance}%
...
{\manualInstrumentedBookstoreApplicationDir/src/kicker/examples/userguide/ch2bookstore/manual/Bookstore.java}
...
In this example, the filesystem writer is used---\Kicker{} uses this writer by default when no other writer is specified, %
...
{\manualInstrumentedBookstoreApplicationDir/src/kicker/examples/userguide/ch2bookstore/manual/CRM.java}
...
\noindent If everything worked correctly, a new directory for the monitoring data with a name similar to \dir{kicker-20120402-163314855-UTC-myHost-KIEKER-SINGLETON/} is created % in the default temporary directory
...
In \Kicker's default configuration, the log directory can be found in the default temporary directory: %
...
The exact location of the created monitoring log is reported in \Kicker's console output (see for example Appendix~\ref{sec:appendix:manualInstrumentation:output}).
...
\dirtree{%
.1 \DirInDirTree{/tmp/}.
.2 \DirInDirTree{kicker-20130910-120352847-UTC-myHost-KIEKER-SINGLETON/}.
.3 kicker.map.
.3 kicker-20120402-163314882-UTC--000-Thread-1.dat.
}
\end{graybox}
...
\setBashListing
\lstinputlisting[caption=kicker-20130910-120352862-UTC-000-Thread-1.dat (excerpt), firstline=1, lastline=2, label=lst:exampledat]%
{ch2-quickstart-example/kicker-20130910-120352847-UTC-myHost-KIEKER-SINGLETON/kicker-20130910-120352862-UTC-000-Thread-1.dat}
...
{ch2-quickstart-example/kicker-20130910-120352847-UTC-myHost-KIEKER-SINGLETON/kicker.map}

\noindent By the end of this section, two Java classes of the Bookstore application %
have been manually instrumented using \KickerMonitoringPart{} and at least one %
...
this monitoring data with \KickerAnalysisPart{}.

%%
\section{Analysis with \KickerAnalysisPart{}}\label{sec:example:analysis}
...
analyzed with \KickerAnalysisPart{}. %
...
the full potential of \Kicker{}. For more detail, read %
Chapter~\ref{chap:componentsAnalysis} to learn which plugins, i.e., readers %
and filters, are included in \Kicker{}, how to use them, and how to develop %
custom plugins. %
Chapter~\ref{chap:aspectJ} presents the \KickerTraceAnalysis{} tool, which %
is also based on \KickerAnalysisPart{}.
\KickerAnalysisPart{} has a dependency to the Eclipse Modeling Framework~(EMF).%
...
be found in \Kicker's \file{lib/} directory) need to be added to the classpath. %
...
\Kicker{}'s file system reader (\class{FSReader}) reads monitoring records %
...
\KickerAnalysisPart{} pipe-and-filter configurations can %
be created programmatically, i.e., by configuring, instantiating, and %
connecting the plugins in a Java program.%
\footnote{As an alternative, a web-based user interface is available for \Kicker{} \cite{KickerWebSite}} %
...
{\manualInstrumentedBookstoreApplicationDir/src/kicker/examples/userguide/ch2bookstore/manual/BookstoreAnalysisStarter.java}
