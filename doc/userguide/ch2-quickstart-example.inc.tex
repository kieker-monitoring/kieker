...
\\
\NOTIFYBOX{Windows comes with two commando interpreters called \texttt{cmd.exe} and \texttt{command.com}. Only the first one is able to handle wildcards correctly. So we recommend using \texttt{cmd.exe} for these examples.}
...
Listing~\ref{lst:cuttingBookstore} shows the instrumentation of the \class{Bookstore} class and its method \method{searchBook()}. In the lines 12 and 13, the monitoring controller is instantiated. It provides the monitoring service for the instrumentation.
...
lines~20 and~22 of Listing~\ref{lst:cuttingCRM}. Not shown in the listing is the instantiation of the monitoring controller. However, it is done in the same way as illustrated in Listing~\ref{lst:cuttingBookstore}. %And the complete example files can be found in the samples directory.
Finally, a record is created (see lines 23--26) and stored by calling the monitoring controller (see line 27).

\setJavaCodeListing
\lstinputlisting[firstline=17, firstnumber=17, lastline=28, caption=Instrumentation of the \method{getBook()} call in CRM.java, label=lst:cuttingCRM]%
...
\lstinputlisting[float,caption=Consumer.java, label=lst:Consumer]{\manualInstrumentedBookstoreApplicationDir/src/bookstoreApplication/Consumer.java}
...
This threshold is set during construction of the \class{Consumer} class (see lines 13--15). 

For the data analysis, the method \method{newMonitoringRecord()} (see lines 23--40) is used. %
This method is called for every monitoring record. At first, the method tests if %
the monitoring record are really \class{OperationExecutionRecord} instances, as %
these are the only record types it can process. Then the methods calculates the %
execution time of one recorded \method{getBook()} call. If the method call takes %
...
\setJavaCodeListing       
\lstinputlisting[caption=BookstoreAnalysisStarter.java,label=lst:BookstoreAnalysisStarter]%
...
The analysis started by calling its \method{run} method (line~27). %
