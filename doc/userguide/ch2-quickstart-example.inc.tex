...
The directory structure of the Bookstore example is shown in Figure~\ref{fig:PlainBookstoreExample} and comprises four Java classes in its source directory \dir{src/bookstoreApplication/} which are explained in detail below.
...
.5 \DirInDirTree{bookstoreApplication/}.
.6 Bookstore.java.
...
\NOTIFYBOX{The Java sources of the uninstrumented Bookstore application can be found in the \file{\plainBookstoreApplicationDirDistro{}/} directory.}

\quad\

\enlargethispage{1cm}

\noindent The class \class{BookstoreStarter}, shown in Listing~\ref{lst:class:BookstoreStarter}, contains the application's \method{main} method, i.e., the program start routine. It initializes the \class{Bookstore} and issues five search requests by calling the \method{searchBook} method of the \object{bookstore} object.

\setJavaCodeListing

\lstinputlisting[caption=BookstoreStarter.java,label=lst:class:BookstoreStarter,firstline=21,firstnumber=21]{\plainBookstoreApplicationDir/src/bookstoreApplication/BookstoreStarter.java}
...
\lstinputlisting[caption=Bookstore.java,label=lst:class:Bookstore,firstline=21,firstnumber=21]{\plainBookstoreApplicationDir/src/bookstoreApplication/Bookstore.java}
...
\lstinputlisting[caption=CRM.java,label=lst:class:CRM,firstline=21,firstnumber=21]{\plainBookstoreApplicationDir/src/bookstoreApplication/CRM.java}
...
\lstinputlisting[caption=Catalog.java,label=lst:class:Catalog,firstline=21,firstnumber=21]{\plainBookstoreApplicationDir/src/bookstoreApplication/Catalog.java}
...
The file \file{\commonsLoggingJar} is located in the \dir{\KiekerDir/lib/} directory and has to be copied to the \dir{lib/} directory of the example application. The final layout of the example directories is illustrated in Figure~\ref{fig:KiekerBookstoreExample}.
...
\noindent The attributes relevant to this part are \method{className} and \method{operationName} %
for the class and method name, as well as \method{tin} and \method{tout} for %
the timestamp before and after the call of the instrumented method. 

\enlargethispage{0.7cm}

Listing~\ref{lst:cuttingBookstore} shows the instrumentation of the \class{Bookstore} class and its method \method{searchBook()}. In the lines 32 and 33, the monitoring controller is instantiated. It provides the monitoring service for the instrumentation.
...
lines~40 and~42 of Listing~\ref{lst:cuttingCRM}. Not shown in the listing is the instantiation of the monitoring controller. However, it is done in the same way as illustrated in Listing~\ref{lst:cuttingBookstore}. %And the complete example files can be found in the samples directory.
Finally, a record is created (see lines 43--46) and stored by calling the monitoring controller (see line 47).

\setJavaCodeListing
\lstinputlisting[firstline=37, firstnumber=37, lastline=48, caption=Instrumentation of the \method{getBook()} call in CRM.java, label=lst:cuttingCRM]%
{\manualInstrumentedBookstoreApplicationDir/src/bookstoreApplication/CRM.java}

% \enlargethispage{0.5cm}

\noindent %
The next step after instrumenting the code is running the instrumented application. Listing~\ref{lst:bookstoreStarterLinux} shows the two commands to compile and run the application under \UnixLikeSystems{}. Listing \ref{lst:bookstoreStarterWin} shows the same commands for Windows. The expected working directory is the base directory of this example, i.e. %
...
\noindent If everything worked correctly, a new directory for the monitoring data with a name similar to \dir{kieker-20110427-142244899-UTC-Kaapstad-KIEKER-SINGLETON/} is created % in the default temporary directory
...
\dirtree{%
.1 \DirInDirTree{/tmp/}.
.2 \DirInDirTree{kieker-20110427-142244899-UTC-Kaapstad-KIEKER-SINGLETON/}.
.3 kieker.map.
.3 kieker-20110427-142244920-UTC-Thread-1.dat.
}
\end{graybox}
...
\setBashListing
\lstinputlisting[caption=kieker-20110427-142244920-UTC-Thread-1.dat (excerpt), firstline=1, lastline=2, label=lst:exampledat]%
...
.3 \DirInDirTree{ch2--manual-instrumentation/}.
.4 \DirInDirTree{build/}\DTcomment{Directory for the Java class files}. 
.4 \DirInDirTree{lib/}\DTcomment{Directory for the required libraries}.
.5 \ldots. 
.5 \mainJar.
.4 \DirInDirTree{src/}\DTcomment{The directory for the source code files}.
.5 \DirInDirTree{bookstoreApplication/}.
...
Listing \ref{lst:Consumer} on page~\pageref{lst:Consumer} shows the content of the \file{Consumer.java} file which implements the \class{IMonitoringRecordConsumerPlugin} interface. This is the standard interface for the consumer of \Kieker{} monitoring records. The consumer is part of the \KiekerAnalysisPart{} component. It processes records provided by the monitoring readers (see Chapter~\ref{chap:introduction}). %For details on the composition of the \Kieker{} framework).

\setJavaCodeListing       

\lstinputlisting[float,caption=Consumer.java, label=lst:Consumer,firstline=21,firstnumber=21]{\manualInstrumentedBookstoreApplicationDir/src/bookstoreApplication/Consumer.java}
...
This threshold is set during construction of the \class{Consumer} class (see lines 33--35). 

\pagebreak

For the data analysis, the method \method{newMonitoringRecord()} (see lines 43--60) is used. %
This method is called for every monitoring record. At first, the method tests if %
the monitoring record is an \class{OperationExecutionRecord}, as %
this is the only record type it can process. Then the method calculates the %
response time of one recorded \method{getBook()} call. If the method call takes %
...
In this case the main program is located in the \file{BookstoreAnalysisStarter.java} %
file shown in Listing~\ref{lst:BookstoreAnalysisStarter}.

\setJavaCodeListing       
\lstinputlisting[caption=BookstoreAnalysisStarter.java,label=lst:BookstoreAnalysisStarter,firstline=21,firstnumber=21]%
...
\class{Consumer} is instantiated and the maximum response time is set to 1.9 milliseconds. %3 nanoseconds. %
...
The analysis is started by calling its \method{run} method (line~47). %
