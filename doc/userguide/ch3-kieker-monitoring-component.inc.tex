...
\chapter{\KickerMonitoringPart{} Component}\label{chap:componentsMonitoring}
...
The \class{MonitoringController} constructs and controls a \KickerMonitoringPart{} %
instance.

\enlargethispage{1cm}

\begin{figure}[h]\centering % 
\includegraphics[scale=0.7]{images/kicker_monitoringControlleruserguide-simplified}
...
\method{getTime} returns a timestamp in nanoseconds. \Kicker{}'s default %
...
For this purpose, \Kicker{} supports special monitoring probes, called samplers. %
...
\setJavaCodeListing
\lstinputlisting[firstline=53, lastline=70, firstnumber=53, caption=Method \method{sample} from MemSwapUsageSampler.java, label=listing:sigarSamplerMethod]{\kickerSrcDir/monitoring/kicker/monitoring/sampler/sigar/samplers/MemSwapUsageSampler.java}
...
by the file \file{kicker.monitoring.adaptive\-Monitoring.conf} in the %
...
in the above file. For example, \Kicker's probes for CPU and memory make use of %
this mechanism.

By default, \Kicker{}'s adaptive monitoring feature is deactivated. It can be %
...
is probably the easiest way to get started. Just keep in mind to add \Kicker{} to the classpath when calling \class{jconsole} so that the MBean works correctly. Figure~\ref{fig:monitoringController:MBean:jconsole} %
...
\section{\KickerMonitoringPart{} Configuration}\label{sec:monitoring:configuration}

\KickerMonitoringPart{} instances can be configured by properties files, %
...
\file{\monitoringPropertiesFile} in the directory \dir{\KickerDir/META-INF/} of %
...
Figure~\ref{fig:KickerCommunicationDiagram} illustrates this life cycle of a monitoring %
record. %

In Chapter~\ref{chap:example}, we've already introduced and used the monitoring %
record type \class{OperationExecutionRecord}. \Kicker{} allows to use custom %
...
this value. \KickerMonitoringPart{} provides the abstract class %
...
\includegraphics[scale=0.75]{images/kicker_MyRTRecord-modified}
...
\setJavaCodeListing
\lstinputlisting[caption=MyResponseTimeRecord.java, label=listing:MyRecord,firstline=27,firstnumber=27]{\customComponentsBookstoreApplicationDir/src/kicker/examples/userguide/ch3and4bookstore/MyResponseTimeRecord.java}
...
% \pagebreak
\lstinputlisting[firstline=32, lastline=40, firstnumber=32, caption=Excerpt from Bookstore.java, label=listing:cuttingBookstore]{\customComponentsBookstoreApplicationDir/src/kicker/examples/userguide/ch3and4bookstore/Bookstore.java}
...
\lstinputlisting[firstline=24, lastline=25, firstnumber=24, caption=Singleton instance of the monitoring controller stored in a final static variable (excerpt from Bookstore.java), label=listing:cuttingBookstore:finalStaticController]{\customComponentsBookstoreApplicationDir/src/kicker/examples/userguide/ch3and4bookstore/Bookstore.java}
...
Chapter~\ref{chap:aspectJ} describes the \Kicker{} probes based on these technologies allowing to %
...
already implemented in \KickerMonitoringPart{}. The available properties for the %
...
\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.7]{images/kicker_writerimplsuserguide-modified}
...
\setJavaCodeListing
\lstinputlisting[caption=MyPipeWriter.java, label=listing:MyWriter,firstline=23,firstnumber=23]{\customComponentsBookstoreApplicationDir/src/kicker/examples/userguide/ch3and4bookstore/MyPipeWriter.java}
...
\KickerMonitoringPart{} configuration property (Section~\ref{chap:componentsMonitoring}) %
...
{\customComponentsBookstoreApplicationDir/META-INF/kicker.monitoring.properties}
...
used by the pipe. This is the way, \Kicker{}'s \class{PipeWriter} works. %
