...
continuous monitoring in production systems inducing only a very low overhead. Further information on the overhead caused by \Kieker{} is provided at \url{http://kieker.sourceforge.net/overhead-evaluation/}.
...
\includegraphics[width=0.98\textwidth]{images/kiekerComponentDiagram-woCloud-bw-w-record-newNames-withTraceAnalysis-colors}
...
data---also in real-time.
 \item A \textbf{time source} utilizing Java's \method{System.nanoTime()} method.
...
on the licenses. The \dir{lib/} directory of the release archives contains a %
