...
\noindent The \KiekerMonitoringPart{} component is responsible for program instrumentation, data collection and logging. Its core is \class{MonitoringController} which %
%receives the monitoring data in so-called monitoring records from monitoring probes, and writes these %
receives the monitoring data %
records into the configured monitoring log. % employing a monitoring log writer. 
%
The component \KiekerAnalysisPart{} is responsible for reading, analyzing and visualizing the monitoring data. Its core is the \class{AnalysisController} which manages the life-cycle of the monitoring log reader and all analysis plugins.
