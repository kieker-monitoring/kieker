...
/**
 * Test API of {@link kieker.common.record.flow.trace.concurrency.monitor.MonitorRequestEvent}.
 * 
 * @author API Checker
 * 
 * @since 1.12
...
public class TestMonitorRequestEventPropertyOrder extends AbstractKiekerTest {
	...
	/**
	 * Test property order processing of {@link kieker.common.record.flow.trace.concurrency.monitor.MonitorRequestEvent} constructors and
	...
	public void testMonitorRequestEventPropertyOrder() { // NOPMD
		...
		final ByteBuffer inputBuffer = APIEvaluationFunctions.createByteBuffer(MonitorRequestEvent.SIZE, 
			this.makeStringRegistry(), values);
					
		final MonitorRequestEvent recordInitParameter = new MonitorRequestEvent(
			PROPERTY_TIMESTAMP,
			PROPERTY_TRACE_ID,
			PROPERTY_ORDER_INDEX,
			PROPERTY_LOCK_ID
		);
		final MonitorRequestEvent recordInitBuffer = new MonitorRequestEvent(DefaultValueDeserializer.instance(), inputBuffer, this.makeStringRegistry());
