...
/**
 * Tests for kieker.common.record.flow.trace.operation.object.BeforeOperationObjectEvent records.
 * 
 * @author Reiner Jung
 * 
 * @since 1.10
 */
public class TestBeforeOperationObjectEventPropertyOrder extends AbstractKiekerTest {
	 ...
	 * Empty constructor.
	 */
	public TestBeforeOperationObjectEventPropertyOrder() {}

	/**
	 * Test property order processing of CPUUtilizationRecord constructors.
	 */
	@Test
	public void testBeforeOperationObjectEventPropertyOrder() { // NOPMD
		final IRegistry<String> stringRegistry = this.makeStringRegistry();
		final ByteBuffer inputBuffer = this.createByteBuffer(BeforeOperationObjectEvent.SIZE, 
			this.makeStringRegistry(),
			PROPERTY_TIMESTAMP, PROPERTY_TRACE_ID, PROPERTY_ORDER_INDEX, PROPERTY_OPERATION_SIGNATURE, PROPERTY_CLASS_SIGNATURE, PROPERTY_OBJECT_ID);
		final Object[] values = { PROPERTY_TIMESTAMP, PROPERTY_TRACE_ID, PROPERTY_ORDER_INDEX, PROPERTY_OPERATION_SIGNATURE, PROPERTY_CLASS_SIGNATURE, PROPERTY_OBJECT_ID };
					
		final BeforeOperationObjectEvent recordInitParameter = new BeforeOperationObjectEvent(PROPERTY_TIMESTAMP, PROPERTY_TRACE_ID, PROPERTY_ORDER_INDEX, PROPERTY_OPERATION_SIGNATURE, PROPERTY_CLASS_SIGNATURE, PROPERTY_OBJECT_ID);
		final BeforeOperationObjectEvent recordInitBuffer = new BeforeOperationObjectEvent(inputBuffer, this.makeStringRegistry());
		final BeforeOperationObjectEvent recordInitArray = new BeforeOperationObjectEvent(values);
		
		this.assertBeforeOperationObjectEvent(recordInitParameter);
		this.assertBeforeOperationObjectEvent(recordInitBuffer);
		this.assertBeforeOperationObjectEvent(recordInitArray);
		...
		final ByteBuffer outputBufferParameter = ByteBuffer.allocate(BeforeOperationObjectEvent.SIZE);
		...
		final ByteBuffer outputBufferBuffer = ByteBuffer.allocate(BeforeOperationObjectEvent.SIZE);
		...
		final ByteBuffer outputBufferArray = ByteBuffer.allocate(BeforeOperationObjectEvent.SIZE);
	...
	/**
	 * Assertions for BeforeOperationObjectEvent.
	 */
	private void assertBeforeOperationObjectEvent(final BeforeOperationObjectEvent record) {
