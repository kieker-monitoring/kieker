...
/**
 * Test API of {@link kieker.common.record.flow.trace.operation.constructor.AfterConstructorEvent}.
 * 
 * @author API Checker
 * 
 * @since 1.12
 */
public class TestAfterConstructorEventPropertyOrder extends AbstractKiekerTest {
	 ...
	 * Empty constructor.
	 */
	public TestAfterConstructorEventPropertyOrder() {
		// Empty constructor for test class.
	}

	/**
	 * Test property order processing of {@link kieker.common.record.flow.trace.operation.constructor.AfterConstructorEvent} constructors and
	 * different serialization routines.
	 */
	@Test
	public void testAfterConstructorEventPropertyOrder() { // NOPMD
		...
		final ByteBuffer inputBuffer = APIEvaluationFunctions.createByteBuffer(AfterConstructorEvent.SIZE, 
			this.makeStringRegistry(), values);
					
		final AfterConstructorEvent recordInitParameter = new AfterConstructorEvent(
			PROPERTY_TIMESTAMP,
			PROPERTY_TRACE_ID,
			PROPERTY_ORDER_INDEX,
			PROPERTY_OPERATION_SIGNATURE,
		...
		final AfterConstructorEvent recordInitBuffer = new AfterConstructorEvent(DefaultValueDeserializer.instance(), inputBuffer, this.makeStringRegistry());
		final AfterConstructorEvent recordInitArray = new AfterConstructorEvent(values);
		
		this.assertAfterConstructorEvent(recordInitParameter);
		this.assertAfterConstructorEvent(recordInitBuffer);
		this.assertAfterConstructorEvent(recordInitArray);
		...
		final ByteBuffer outputBufferParameter = ByteBuffer.allocate(AfterConstructorEvent.SIZE);
		...
		final ByteBuffer outputBufferBuffer = ByteBuffer.allocate(AfterConstructorEvent.SIZE);
		...
		final ByteBuffer outputBufferArray = ByteBuffer.allocate(AfterConstructorEvent.SIZE);
	...
	/**
	 * Assertions for AfterConstructorEvent.
	 */
	private void assertAfterConstructorEvent(final AfterConstructorEvent record) {
