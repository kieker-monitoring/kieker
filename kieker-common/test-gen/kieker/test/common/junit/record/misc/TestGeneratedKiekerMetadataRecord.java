...
public class TestGeneratedKiekerMetadataRecord extends AbstractGeneratedKiekerTest {
	...
	public void testToArray() { // NOPMD (assert missing)
		for (int i=0;i<ARRAY_LENGTH;i++) {
			...
			// check values
			Assert.assertEquals("KiekerMetadataRecord.version values are not equal.", STRING_VALUES.get(i % STRING_VALUES.size()) == null?kieker.common.util.Version.getVERSION():STRING_VALUES.get(i % STRING_VALUES.size()), record.getVersion());
			...
			Assert.assertEquals("Array value [0] " + values[0] + " does not match the desired value " + STRING_VALUES.get(i % STRING_VALUES.size()),
				STRING_VALUES.get(i % STRING_VALUES.size()) == null?kieker.common.util.Version.getVERSION():STRING_VALUES.get(i % STRING_VALUES.size()), values[0]
			);
