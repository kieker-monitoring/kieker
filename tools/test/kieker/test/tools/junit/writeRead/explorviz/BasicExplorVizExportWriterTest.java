 ...
 * Kieker ExplorVizExportWriter correctly.
 *
...
public class BasicExplorVizExportWriterTest {
	...
	private static final int CONNECTION_TIMEOUT_IN_MS = BasicExplorVizExportWriterTest.WHOLE_TEST_TIMEOUT - 2000;
	...
	@Test(timeout = BasicExplorVizExportWriterTest.WHOLE_TEST_TIMEOUT)
	public void testExplorvizCommunication() throws Exception {
		// define records to be triggered by the test probe
		final List<IMonitoringRecord> records = BasicExplorVizExportWriterTest.TEST_DATA_REPOSITORY
				.newTestEventRecords();
		...
		config.setProperty(ConfigurationConstants.WRITER_CLASSNAME, ExplorVizTcpWriter.class.getName());
		...
		Await.awaitTcpPortIsReachable(hostname, Integer.parseInt(port),
				BasicExplorVizExportWriterTest.CONNECTION_TIMEOUT_IN_MS);
		...
		// It is impossible to get to know whether waitForTermination() has returned
		// normally or due to a timeout.
		// Hence, we use JUnit's timeout at method level to fail this test upon a
		// deadlock.
		monitoringController.waitForTermination(BasicExplorVizExportWriterTest.UNLIMITED_TIMEOUT);
		analysis.waitForTermination(BasicExplorVizExportWriterTest.UNLIMITED_TIMEOUT);
	...
	private void inspectRecords(final List<IMonitoringRecord> eventsFromMonitoringLog,
			final List<IMonitoringRecord> eventsPassedToController) throws Exception {
			...
			if (eventFromLog instanceof CustomAfterOperationFailedEvent) {
				...
				Assert.assertEquals("Unexpected set of records - sent record does not match received record",
						(sentRecord.getTimestamp() + ", " + sentRecord.getTraceId() + ", " + sentRecord.getOrderIndex()
								+ ", " + sentRecord.getCause()),
						(receivedRecord.getTimestamp() + ", " + receivedRecord.getTraceId() + ", "
								+ receivedRecord.getOrderIndex() + ", " + receivedRecord.getCause()));
				...
				Assert.assertEquals("Unexpected set of records - sent record does not match received record",
						(sentRecord.getTimestamp() + ", " + sentRecord.getTraceId() + ", "
								+ sentRecord.getOrderIndex()),
						(receivedRecord.getTimestamp() + ", " + receivedRecord.getTraceId() + ", "
								+ receivedRecord.getOrderIndex()));
				...
				Assert.assertEquals("Unexpected set of records - sent record does not match received record",
						...
						(receivedRecord.getTimestamp() + ", " + receivedRecord.getTraceId() + ", "
								+ receivedRecord.getOrderIndex() + ", " + receivedRecord.getOperationSignature() + ", "
								+ receivedRecord.getClassSignature()));
				...
				Assert.assertEquals("Unexpected set of records - sent record does not match received record",
						(sentRecord.getId() + ", " + sentRecord.getString()),
						(receivedRecord.getId() + ", " + receivedRecord.getString()));
			}
		}
	}
