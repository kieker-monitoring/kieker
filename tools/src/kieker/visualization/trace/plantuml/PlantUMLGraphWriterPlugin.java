...
/**
 * Generic graph writer plugin to generate graph specifications and save them to
 * disk. This plugin uses
 * a formatter registry (see {@link #FORMATTER_REGISTRY}) to determine the
 * appropriate formatter for a
 * given graph.
 *
...
public class PlantUMLGraphWriterPlugin extends AbstractConsumerStage<AbstractGraph<?, ?, ?>> {
    ...
    static {
        FORMATTER_REGISTRY.put(ComponentAllocationDependencyGraph.class,
                ComponentAllocationDependencyGraphFormatter.class);
        FORMATTER_REGISTRY.put(ComponentAssemblyDependencyGraph.class, ComponentAssemblyDependencyGraphFormatter.class);
        FORMATTER_REGISTRY.put(OperationAllocationDependencyGraph.class,
                OperationAllocationDependencyGraphFormatter.class);
     ...
     *                       plot loops
     */
    public PlantUMLGraphWriterPlugin(final String outputPathName, final String outputFileName,
            final boolean includeWeights, final boolean useShortLabels,
    ...
    private static void handleInstantiationException(final Class<?> graphClass, final Class<?> formatterClass,
            final Exception exception) {
        throw new GraphFormattingException(
                String.format(INSTANTIATION_ERROR_MESSAGE_TEMPLATE, formatterClass.getName(), graphClass.getName()),
                exception);
    }
    ...
    private static AbstractGraphFormatter<?> createFormatter(final AbstractGraph<?, ?, ?> graph) {
        ...
        if (formatterClass == null) {
            throw new GraphFormattingException(
                    String.format(NO_SUITABLE_FORMATTER_MESSAGE_TEMPLATE, graph.getClass().getName()));
        }
        ...
        // This should never happen, because all catch clauses indirectly throw
        // exceptions
        return null;
    }
    ...
    /**
     * Formats a given graph and saves the generated specification to disk. The file
     * name to save the output to is specified by a the configuration options
     * {@link #CONFIG_PROPERTY_NAME_OUTPUT_PATH_NAME} and
     * {@link #CONFIG_PROPERTY_NAME_OUTPUT_FILE_NAME}.
     *
     * @param graph
     *              The graph to save
     */
    ...
    protected void execute(final AbstractGraph<?, ?, ?> graph) throws Exception {
        ...
        final String specification = graphFormatter.createFormattedRepresentation(graph, this.includeWeights,
                this.useShortLabels, this.plotLoops);
