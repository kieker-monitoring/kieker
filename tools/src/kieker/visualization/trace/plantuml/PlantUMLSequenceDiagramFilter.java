...
public class PlantUMLSequenceDiagramFilter extends AbstractMessageTraceProcessingFilter {
     ...
     *                     use short labels
     */
    public PlantUMLSequenceDiagramFilter(final SystemModelRepository repository, final SDModes sdmode,
    ...
    /**
     * Processes a message trace and generates the corresponding PlantUML sequence
     * diagram.
     */
    @Override
    protected void execute(final MessageTrace mt) throws Exception {
        try {
            final File file = writePlantUMLForMessageTrace(mt, this.sdmode,
                    this.outputFnBase + "-" + mt.getTraceId() + ".puml", this.shortLabels);
    ...
    /**
     * Converts a given message trace to its PlantUML representation and writes it
    ...
    private static void generateAssemblyParticipants(final Iterable<AbstractMessage> messages,
        ...
        for (final AbstractMessage me : messages) {
            ...
            if (!plottedComponentIds.contains(senderComponent.getId())) {
                final String senderDotId = PlantUMLUtils.dotId(senderComponent.getId());
                final String senderLabel = PlantUMLUtils.label(null,
            ...
            if (!plottedComponentIds.contains(receiverComponent.getId())) {
                final String receiverDotId = PlantUMLUtils.dotId(receiverComponent.getId());
                final String receiverLabel = PlantUMLUtils.label(null,
        ...
        switch (sdMode) {
            ...
            case ASSEMBLY:
                return PlantUMLUtils
                        .dotId(me.getSendingExecution().getAllocationComponent().getAssemblyComponent().getId());
            default:
                LOGGER.error(INVALID_MODE_MESSAGE, sdMode);
                return PlantUMLUtils.dotId(-1);
        }
    ...
    private static String getReceiverDotId(final AbstractMessage me, final SDModes sdMode) {
        ...
        switch (sdMode) {
            ...
            case ASSEMBLY:
                return PlantUMLUtils
                        .dotId(me.getReceivingExecution().getAllocationComponent().getAssemblyComponent().getId());
            default:
                LOGGER.error(INVALID_MODE_MESSAGE, sdMode);
                return PlantUMLUtils.dotId(-1);
        }
