...
public abstract class AbstractTraceAnalysisConfiguration extends Configuration {
        ...
        public AbstractTraceAnalysisConfiguration(final TraceAnalysisParameters parameters,
                ...
                final String outputPath = (parameters.getOutputDir() != null)
                                ? parameters.getOutputDir().toString() + File.separator
                                : "";
                ...
                if (parameters.getIgnoreExecutionsBeforeDate() == null) {
                        parameters
                                        .setIgnoreExecutionsBeforeDate(Long.parseLong(
                                                        TimestampFilter.CONFIG_PROPERTY_VALUE_MIN_TIMESTAMP));
                } else {
                ...
                if (parameters.getIgnoreExecutionsAfterDate() == null) {
                        parameters
                                        .setIgnoreExecutionsAfterDate(Long.parseLong(
                                                        TimestampFilter.CONFIG_PROPERTY_VALUE_MAX_TIMESTAMP));
                } else {
                        if (AbstractTraceAnalysisConfiguration.LOGGER.isInfoEnabled()) {
                ...
                final Set<Long> selectedTraces = (select == null) ? new TreeSet<>() : new TreeSet<>(select);
                ...
                final IEventMatcher<? extends IFlowRecord> flowRecordMatcher = new ImplementsEventMatcher<>(
                                IFlowRecord.class,
                                null);
                ...
                final Distributor<TraceEventRecords> validTracesDistributor = new Distributor<>(
                ...
                this.invalidEventRecordTraceCounter = new InvalidEventRecordTraceCounter(
                                parameters.isIgnoreInvalidTraces());
                ...
                final Distributor<ExecutionTrace> executionTraceDistributor = new Distributor<>(
                                new CopyByReferenceStrategy());
                final Distributor<InvalidExecutionTrace> invalidExecutionTraceDistributor = new Distributor<>(
                                new CopyByReferenceStrategy());
                final Distributor<MessageTrace> messageTraceDistributor = new Distributor<>(
                                new CopyByReferenceStrategy());
                ...
                this.connectPorts(validTracesDistributor.getNewOutputPort(),
                                this.validEventRecordTraceCounter.getInputPort());
                ...
                this.connectPorts(this.traceEventRecords2ExecutionAndMessageTraceStage.getExecutionTraceOutputPort(),
                                executionTraceMerger.getNewInputPort());
                this.connectPorts(
                                this.traceEventRecords2ExecutionAndMessageTraceStage
                ...
                this.connectPorts(invalidExecutionTraceMerger.getOutputPort(),
                                invalidExecutionTraceDistributor.getInputPort());
                ...
                if (parameters.isPrintInvalidExecutionTraces()) {
                        this.createInvalidExecutionTraceWriterSink(systemRepository, invalidExecutionTraceDistributor,
                                        pathPrefix);
                }
                ...
                if (parameters.isPlotDeploymentSequenceDiagrams()) {
                        this.createPlotSequenceDiagrams(systemRepository, messageTraceDistributor, pathPrefix,
                                        VisualizationConstants.ALLOCATION_SEQUENCE_DIAGRAM_FN_PREFIX,
                                        SDModes.ALLOCATION,
                ...
                if (parameters.getPlotDeploymentComponentDependencyGraph() != null) {
                        this.createPlotDeploymentComponentDependencyGraph(systemRepository, messageTraceDistributor,
                                        ...
                                        outputPath, filePrefix, true, parameters.isShortLabels(),
                ...
                if (parameters.isPlotContainerDependencyGraph()) {
                        this.createPlotContainerDependencyGraph(systemRepository, messageTraceDistributor, outputPath,
                                        filePrefix, true,
                                        parameters.isShortLabels(),
                                        parameters.isIncludeSelfLoops());
                }
                ...
                if (parameters.isPlotAggregatedDeploymentCallTree()) {
                        this.createPlotAggregatedDeploymentCallTree(systemRepository, messageTraceDistributor,
                                        pathPrefix, true,
                                        parameters.isShortLabels());
                }

                if (parameters.isPlotAggregatedAssemblyCallTree()) {
                        this.createAggrAssemblyCompOpCallTreeFilter(systemRepository, messageTraceDistributor,
                                        pathPrefix, true,
                                        parameters.isShortLabels());
                }
        }
        ...
        protected abstract void createPlotDeploymentComponentDependencyGraph(
                        final SystemModelRepository systemRepository,
        ...
        protected abstract void createPlotDeploymentOperationDependencyGraph(
                        final SystemModelRepository systemRepository,
                        final Distributor<MessageTrace> messageTraceDistributor, final List<String> decoratorList,
                        final String outputPathName, final String outputFileName,
                        final boolean includeWeights, final boolean shortLabels,
        ...
        private void createComponentPrintMsgTrace(final SystemModelRepository systemRepository,
                ...
                try {
                        final MessageTraceWriterFilter messageWriterStink = new MessageTraceWriterFilter(
                                        systemRepository,
                                        new File(pathPrefix
                                                        + AbstractTraceAnalysisConfiguration.MESSAGE_TRACES_FN_PREFIX
                                                        + AbstractTraceAnalysisConfiguration.TXT_SUFFIX));
                        this.connectPorts(messageTraceDistributor.getNewOutputPort(),
                                        messageWriterStink.getInputPort());
                } catch (final IOException e) {
                        if (AbstractTraceAnalysisConfiguration.LOGGER.isErrorEnabled()) {
                                AbstractTraceAnalysisConfiguration.LOGGER.error(String.format(
                                                "Error initializing %s cause %s",
        ...
        private void createComponentPrintExecTrace(final SystemModelRepository systemRepository,
                        ...
                        final String pathPrefix) {
                try {
                        final ExecutionTraceWriterFilter executionWriterStink = new ExecutionTraceWriterFilter(
                                        systemRepository,
                                        new File(pathPrefix
                                                        + AbstractTraceAnalysisConfiguration.EXECUTION_TRACES_FN_PREFIX
                                                        + AbstractTraceAnalysisConfiguration.TXT_SUFFIX));
                        this.connectPorts(executionTraceDistributor.getNewOutputPort(),
                                        executionWriterStink.getInputPort());
                } catch (final IOException e) {
                        if (AbstractTraceAnalysisConfiguration.LOGGER.isErrorEnabled()) {
                                AbstractTraceAnalysisConfiguration.LOGGER.error(String.format(
                                                "Error initializing %s cause %s",
        ...
        private void createInvalidExecutionTraceWriterSink(final SystemModelRepository systemRepository,
                        final Distributor<InvalidExecutionTrace> invalidExecutionTraceDistributor,
                        final String pathPrefix) {
                try {
                        final InvalidExecutionTraceWriterSink invalidExecutionTraceWriterSink = new InvalidExecutionTraceWriterSink(
                                        systemRepository,
                                        new File(pathPrefix
                                                        + AbstractTraceAnalysisConfiguration.INVALID_TRACES_FN_PREFIX
                                                        + AbstractTraceAnalysisConfiguration.TXT_SUFFIX));
                        ...
                        if (AbstractTraceAnalysisConfiguration.LOGGER.isErrorEnabled()) {
                                AbstractTraceAnalysisConfiguration.LOGGER
                                                .error(String.format("Error initializing %s cause %s",
                                                                InvalidExecutionTraceWriterSink.class,
        ...
        protected void addDecorators(final List<String> decoratorList, final AbstractDependencyGraphFilter<?> plugin) {
                if (decoratorList != null) {
                        ...
                        while (decoratorIterator.hasNext()) {
                                ...
                                if (VisualizationConstants.RESPONSE_TIME_DECORATOR_FLAG_NS
                                                .equals(currentDecoratorStr)) {
                                        plugin.addDecorator(new ResponseTimeNodeDecorator(TimeUnit.NANOSECONDS));
                                        continue;
                                } else if (VisualizationConstants.RESPONSE_TIME_DECORATOR_FLAG_US
                                                .equals(currentDecoratorStr)) {
                                        plugin.addDecorator(new ResponseTimeNodeDecorator(TimeUnit.MICROSECONDS));
                                        continue;
                                } else if (VisualizationConstants.RESPONSE_TIME_DECORATOR_FLAG_MS
                                                .equals(currentDecoratorStr)) {
                                        plugin.addDecorator(new ResponseTimeNodeDecorator(TimeUnit.MILLISECONDS));
                                        continue;
                                } else if (VisualizationConstants.RESPONSE_TIME_DECORATOR_FLAG_S
                                                .equals(currentDecoratorStr)) {
                                        ...
                                        try {
                                                ...
                                                if (AbstractTraceAnalysisConfiguration.LOGGER.isErrorEnabled()) {
                                                        AbstractTraceAnalysisConfiguration.LOGGER.error(
                                                                        "Failed to parse int value of property threshold(ms) : {}",
                                                                        thresholdStringStr);
                                                }
                                        }
                                } else {
                                        if (AbstractTraceAnalysisConfiguration.LOGGER.isWarnEnabled()) {
                                                AbstractTraceAnalysisConfiguration.LOGGER.warn(
