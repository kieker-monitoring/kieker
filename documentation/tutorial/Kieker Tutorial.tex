...
\end{itemize}
The both parts consist themselves consist each of three subparts which can be both used and extended with own classes. The rough interaction between the different components is described by the following figure.
\begin{figure}[H]
  \begin{center}
    \includegraphics[width=1.0\textwidth]{kiekerCommunications-revisedReArranged-woMonitoringLog-bw.pdf}
    \caption{Interaction between components of \Kieker}
  \end{center}
\end{figure}
The monitoring probes create either the monitoring records directly or by calling the monitoring controller. The monitoring controller uses the monitoring log writers to persist the given record. The persisted informations can later be read by a monitoring log reader who creates a monitoring record again which can then be used in some way by a consumer.\\
The more detailed informations about the components will be described later in this tutorial.
