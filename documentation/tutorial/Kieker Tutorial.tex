...
\begin{enumerate}
 ...
 \item The configuration file has to be stored as well in the ''.jar`` file and \Kieker\ has to be informed of the properties file\footnote{This is done by supply java with an additional parameter. If the properties are stored in the file ''tpmon.properties'' then the parameter is ''-Dtpmon.configuration=META-INF/tpmon.properties``}.
\end{enumerate}
\setJavaCodeListing
