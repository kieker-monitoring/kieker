...
- Download Kieker complete archive https://github.com/kieker-monitoring/kieker/releases/download/2.0.4-SNAPSHOT/kieker-2.0.4-SNAPSHOT-binaries.tar.gz
- Extract the package
- Copy ``kieker-2.0.4-SNAPSHOT-aspectj.jar`` from the ``build/libs/ directory`` into the same directory as your jar file.
- Execute ``java -javaagent:kieker-2.0.4-SNAPSHOT-aspectj.jar -jar MyJar.jar``
...
``java -javaagent:kieker-2.0.4-SNAPSHOT-aspectj.jar -Dkieker.monitoring.configuration=file://path/to/kieker.properties -Dorg.aspectj.weaver.loadtime.configuration=file://path/to/aop.xml -jar MyJar.jar``
